...
var targetStep = 10; // frame to animate to
var images = new Array; // stores all of the frames for quick access
var scrollPos; // scroll position of the window
var totalFrames = 255; // the number of images in the sequence of JPEG files (this could be calculated server-side by scanning the frames folder)
...
// Check the certain offset.top values and then don't change the image



	var section1Top =  0;
	// The top of each section is offset by half the distance to the previous section.
	var section2Top =  $('#internet').offset().top ;
	var section3Top =  $('#wlan').offset().top;
	var section4Top =  $('#usb').offset().top;
	var section5Top =  $('#lan').offset().top;
	var section6Top =  $('#os').offset().top;
	var section7Top =  $('#auszeichnungen').offset().top;


	if($(document).scrollTop() == section1Top ||  section2Top || section3Top ) {
		  var delay = setTimeout(changeFrame, 1000);
	}else {
		cleanTimeout(delay);
		changeFrame();
	}
